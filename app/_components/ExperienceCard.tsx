  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-2 sm:p-4 lg:p-6">
      <motion.div
        ...
        className="w-full max-w-xs sm:max-w-md md:max-w-2xl lg:max-w-4xl bg-white dark:bg-gray-800 rounded-2xl sm:rounded-3xl shadow-xl dark:shadow-2xl dark:shadow-black/20 overflow-hidden border dark:border-gray-700"
      >
        {/* Header Section */}
        <motion.div
        ...
        >
          <motion.div
            className="absolute top-0 right-0 w-16 h-16 sm:w-24 sm:h-24 lg:w-32 lg:h-32 bg-gradient-to-br from-purple-100 to-blue-100 dark:from-purple-900/30 dark:to-blue-900/30 rounded-bl-full opacity-50"
          ...
          <div className="relative">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 sm:mb-6 gap-3 sm:gap-4">
              <div className="flex-1 min-w-0">
                <motion.h1
                  variants={itemVariants}
                  className="text-xl sm:text-2xl lg:text-3xl font-bold font-heading text-gray-900 dark:text-white mb-2 break-words"
                >
                  {title}
                </motion.h1>
                <motion.a
                  variants={itemVariants}
                  href="#"
                  className="text-base sm:text-lg lg:text-xl text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-semibold font-heading transition-colors duration-200 break-words"
                  whileHover={{ x: 4 }}
                >
                  {company}
                </motion.a>
              ...
              <motion.div
                variants={itemVariants}
                className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 rounded-full text-xs sm:text-sm font-medium shrink-0 self-start"
        ...
        {projectLinks.length > 0 && (
          <motion.div
            variants={itemVariants}
            className="px-4 sm:px-6 lg:px-8 pb-4 sm:pb-6"
          >
            <motion.h2
              variants={itemVariants}
              className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4"
            >
              Project Links
            </motion.h2>
            <div className="flex flex-wrap gap-3 sm:gap-4">
              {projectLinks.map((link, index) => {
                ...
                return (
                  ...
                  >
                    <motion.div whileHover={{ rotate: 12, scale: 1.2 }} transition={{ duration: 0.2 }}>
        ...
        >
          <motion.h2
            variants={itemVariants}
            className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white mb-4 sm:mb-6"
          >
          ...
          <div className="space-y-3 sm:space-y-4">
            {achievements.map((achievement, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="flex items-start gap-3 sm:gap-4 group hover:bg-gray-50 dark:hover:bg-gray-700/50 p-3 sm:p-4 rounded-lg sm:rounded-xl transition-all duration-300"
                whileHover={{ x: 4 }}
              >
                <motion.div
